...
{
    public class SqlServer2000Provider : DatabaseProvider
    {
        ...
        {
            string sql = @"select [name] as TableName from sysobjects 
            where xtype = 'U' and status > 0 and year(crdate)>2000 order by [name]";
        ...
        {
            string sql = @"select TableName=[name] from sysobjects 
                           where xtype = 'V' and status > 0 and year(crdate)>2000 order by [name]";
