...
{ 
    public abstract class SqlBuilder : ISqlBuilder
    {
        ...
        {
            return this.FormatParameterList(columnNames, DatabaseType.SqlServer);
        }

        public string FormatParameterList(IList<string> columnNames, DatabaseType databaseType)
        {
            if (columnNames == null || columnNames.Count == 0)
            {
                return string.Empty;
        ...
        public static string FormatColumnAndParameterPairs(IList<string> columnNames, DatabaseType databaseType)
        {
            if (columnNames == null || columnNames.Count == 0)
            {
                return string.Empty;
        ...
        public string Delete<T>(string condition) where T : TableExecutor<T>, new()
        {
        ...
        public string DeleteById<T>(T entity) where T : TableExecutor<T>, new()
        { 
            return Delete<T>(BuildExpressionById(entity));
        }

        public string DeleteByPK<T>(T entity) where T : TableExecutor<T>, new()
        {
        ...
        public SqlCommandParameter CreateSqlUpdateCommandParameter<T>(T entity, string condition) where T : TableExecutor<T>, new()
        {
            if (entity == null)
            {
                throw new ArgumentNullException(MsgResource.InvalidArguments);
        ...
        public virtual SqlCommandParameter CreateSqlInsertCommandParameter<T>(T entity, IList<string> propertyNames,
            IdentityPrimaryKeyInfo identityInfo, bool returnIdentity = true) where T : TableExecutor<T>, new()
        {
            if (propertyNames == null || propertyNames.Count <= 0)
            {
